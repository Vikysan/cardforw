...
const Select = (props) => {
    ...
    useEffect(() => {
      ...
      if (localTopic) {
        setTopics([...new Set((words.filter(item=>item.file===localTopic.file)).map(item=>item.topic))])
  ...
  return (
    <div>
      <select name="File" id="File" onChange={onChange}>
        {files.map((item,index)=><option value={item} key={'f' + index} selected={currentFile===item} >{item}</option>)}
        </select>
        <select name="Topic" id="Topic" onChange={onChangeTopics}>
        {topics.map((item,index)=><option value={item} key={'t'+ index} selected={currentTopic===item}>{item}</option>)}
        </select>
    </div>
  )
}
